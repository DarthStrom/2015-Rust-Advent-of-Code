...
pub fn run() {
    ...
    let monster = Monster::new("Boss", 51, 9);
...
impl fmt::Display for Effect {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_fmt(format_args!(
            "Effect {}t, {:+} armor, {:+} hp/t, {:+} mp/t",
            self.duration, self.armor, self.hpt, self.mpt
        ))
    }
}
...
impl<'a> fmt::Display for Spell<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_fmt(format_args!(
            "Spell {}, self {:+}|{:+}, target {:+}|{:+}",
            self.name, self.hp_self, self.mp_self, self.hp_target, self.mp_target
        ))
    }
}
...
fn simulate_fights(player: &Player, monster: &Monster, min_mp: &mut isize, mp: isize, hard: bool) {
    for spell in SPELLS.iter() {
        ...
        if !player.can_cast(&monster, spell) {
            continue;
        }
